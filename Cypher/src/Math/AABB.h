   ...
   public:
      ...
      T operator[](int32_t index)
      {
         switch (index)
         {
            ...
            case 2: return maxX;
            case 3: return maxY;
            default: throw std::out_of_range("Index out of range for AABB");
         }
      }
...
{
   template <typename T>
   struct tuple_size<Cypher::AABB<T>> : integral_constant<size_t, Cypher::AABB<T>::ELEMENT_COUNT> {};

   template <size_t N, typename T>
   struct tuple_element<N, Cypher::AABB<T>> { using type = T; };

   template <size_t N, typename T>
   constexpr T& get(Cypher::AABB<T>& aabb)
   {
      static_assert(N < Cypher::AABB<T>::ELEMENT_COUNT, "Index out of bounds");
      if constexpr (N == 0)
         return aabb.minX;
   ...
   template <size_t N, typename T>
   constexpr const T& get(const Cypher::AABB<T>& aabb)
   {
      static_assert(N < Cypher::AABB<T>::ELEMENT_COUNT, "Index out of bounds");
      if constexpr (N == 0)
         return aabb.minX;
